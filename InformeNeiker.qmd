...
```{r}
Gene_stats <-rare_table %>% 
  as.data.frame() %>% 
  rownames_to_column("Sample") %>% 
  pivot_longer(names_to = "Gene", values_to = "Abundance", -Sample) %>% 
  group_by(Gene) %>% 
  summarise(MaxDepth = max(Abundance),
            SumDepth = sum(Abundance),
            Freq = sum(Abundance != 0),
            MeanDepth = mean(Abundance),
            SdDepth = sd(Abundance),
            VarDepth = var(Abundance)) 

Gene_stats %>% 
  pivot_longer(names_to = "Variable", values_to = "Value", c(-Gene,-Freq)) %>% 
  ggplot(aes(x = Freq, y = Value)) +
  geom_point() +
  facet_wrap(~Variable, scales = "free") 

Gene_stats %>% 
  ggplot(aes(x = MaxDepth, y = SdDepth)) +
  geom_point()

Gene_stats %>% 
  ggplot(aes(y = MaxDepth, x = SumDepth)) +
  geom_point() +
